"""
ratelimit_middleware.py  [UPDATED — covers authapp, admin_auth, and student_auth]
-----------------------
...
--- student_auth (student portal) ---
student-signup/                             POST        20/h         IP
student-login/                              POST        5/m          IP
batch-create/                               POST        5/h          IP
...
# ---------------------------------------------------------------------------
RATE_LIMIT_RULES = [
    # ── admin_auth endpoints (checked first — most specific) ────────────────
    ('signup-auth',                         ('POST',),           '10/h',  'admin_signup'),
    ('login-auth',                          ('POST',),           '5/m',   'admin_login'),

    # ── student_auth endpoints ───────────────────────────────────────────────
    # Must come before 'signup' and 'login' — their fragments are longer/distinct
    ('student-signup',                      ('POST',),           '20/h',  'student_signup'),
    ('student-login',                       ('POST',),           '5/m',   'student_login'),
    ('batch-create',                        ('POST',),           '5/h',   'batch_create'),

    # ── authapp endpoints ───────────────────────────────────────────────────
    # verify-email is shared across all three apps — one rule covers all of them
